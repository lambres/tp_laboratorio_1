...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
	...
	if(this != NULL && nodeIndex >= 0 && nodeIndex <= ll_len(this) )
	{
		...
		//si pude crear el nodo entro
		if(newNode != NULL)
		{
			//si es el primer indice a agregar
			if (nodeIndex==0)
			{
				//tomo la direccion de memoria el pFirstNode de la LL
				//y lo guardo en el nodo creado
				newNode->pNextNode = this->pFirstNode;
				//a la LL pfirstnode le asigno la direccion de memoria del nodo recien creado
				this->pFirstNode = newNode;
			}
			else
			{
				//en caso de no ser el primero creo un nuevo nodo para poder hacer un swap
				Node* auxNode = (Node*)malloc(sizeof(Node));
				//si pude crearlo
				if(auxNode != NULL)
				{
					//asigno el puntero del nodo anterior al nodo auxiliar
					//el puntero del nodo siguiente de indice lo asigno al nuevo nodo
					//el nodo anterior lo asigno al nodo siguiente
					//ej nodeindex 8
					//getnode obtiene 7 y lo guarda a auxnode
					//pnextnode de auxnode lo asigno a newnode pnextnode
					//el nodo nuevo lo asigno a pnexnode del auxiliar
...
int ll_containsAll(LinkedList* this,LinkedList* this2)
{
   int returnAux = -1;
    if(this != NULL && this2 != NULL)
    {
		...
		{
    		auxElemento = ll_get(this2, j);
			if(auxElemento != NULL && ll_contains(this, auxElemento) == 0)
			{
				returnAux = 0;
				break;
			}
...
int ll_sort(LinkedList* this, int (*pFunc)(void* ,void*), int order)
{
	...
	int i;
	int j;
	void* auxPrimero = NULL;
	void* auxSegundo = NULL;
	int limite = ll_len(this);
	if(this != NULL && pFunc != NULL && (order == 1 || order == 0)) //1 asc 0 desc
	{
		for( i = 0; i < limite-1; i++)
		{
			for( j = i+1;j < limite; j++)
			{
				auxPrimero = ll_get(this,i);
				auxSegundo = ll_get(this,j);
				if((order == 1 && pFunc(auxPrimero,auxSegundo) > 0)||
					(order == 0 && pFunc(auxPrimero,auxSegundo) < 0))
				{
					ll_set(this,i,auxSegundo);
					ll_set(this,j,auxPrimero);
				}
			}
		}
		returnAux = 0;
...
int ll_filter(LinkedList* this,LinkedList* filterList, int (*pFunc)(void*, char*), char* pais)
{
	int returnAux =-1;
	int i;
	void* aux = NULL;
	int limite = ll_len(this);
	if(this != NULL && filterList != NULL && pFunc != NULL && pais != NULL)
	{
		for( i = 0; i < limite; i++)
		{
			aux = ll_get(this,i);
